...
namespace NVNC
{
    ...
    public class VncHost
    {
        ...
        private void DoFrameBufferUpdate(Framebuffer fb, bool incremental, int x, int y, int width, int height)
        {
            //if (incremental)
            //    return;
            Console.WriteLine("TRACE: " +"X: " + x + " Y: " + y + " W: " + fb.Width + " H: " + fb.Height);
            int w = fb.Width;
            int h = fb.Height;
            if ((x < 0) || (y < 0) || (width <= 0) || (height <= 0))
            {
                Console.WriteLine("TRACE: " +"Neg:" + x + ":" + y + ":" + width + ":" + height);
                return;
            }
            if (x + width > w)
            {
                Console.WriteLine("TRACE: " +"Too wide");
                return;
            }
            if (y + height > h)
            {
                Console.WriteLine("TRACE: " +"Too high");
                return;
            }
            Console.WriteLine("TRACE: " +"Bounds OK!");
            ...
            try
            {
                ...
                Console.WriteLine("TRACE: " +list.Count + " rectangles to encode");
                foreach (QuadNode iter in list)
                {
                    Console.WriteLine("TRACE: " +iter.ToString());
                    EncodedRectangle localRect = factory.Build(iter, GetPreferredEncoding());
                    localRect.Encode();

                    rectangles.Add(localRect);
                }
                Console.WriteLine("TRACE: " +"Encoding took: " + tip.Elapsed);
            }
        ...
        public void WriteFrameBufferUpdate(ICollection<EncodedRectangle> rectangles)
        {
            ...
            try
            {
                ...
                Watch.Stop();
                Console.WriteLine("TRACE: " +"Sending took: " + Watch.Elapsed);
            }
            catch (IOException ex)
            {
                Console.WriteLine(ex.Message);
